...
import { getPublicProfile, getPlayerAchievements } from "./nexus-player.functions";
...
export const playerAchievementsQuery = (playerTag: string) =>
  queryOptions({
    queryKey: ["player-achievements", playerTag],
    queryFn: () => getPlayerAchievements({ data: { player_tag: playerTag } }),
    staleTime: 60_000,
    gcTime: 5 * 60_000,
  });
